      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-4 bg-navy-800 text-cream-50 rounded-xl shadow-2xl z-50 border border-gold-400/30"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-full left-0 mb-2 w-80 p-6 bg-white border border-cream-200 rounded-xl shadow-2xl z-50"
          >
            <div className="flex items-center gap-2 mb-3">
              <BookOpen size={16} className="text-gold-600" />
              <span className="text-xs font-bold text-navy-800">Reference [{id}]</span>
            </div>
            <div className="text-sm text-navy-600 leading-relaxed italic">
              {children}
            </div>
            <button 
